...
public class ConfigRulePage extends BasePage{
	...
	public void deleteAllCoverage() {
		int size = common.getTableColValue(CoverageSelectedRow,CoverageSelectedHeader,"Code").size();
		while(size > 0) {
			clickOn(crossMark);
			size--;
		}
	...
	public void enter_encounter_id(String rule) throws ClassNotFoundException, IOException, SQLException {
		String id = "";
		switch(rule) {
			case "PAS": id = ConfigRulesStep.searchEncounterIDPAS("EncounterID");
				break;
			case "PCE": id = ConfigRulesStep.searchEncounterIDPCE("EncounterID");
				break;
			case "SAR": id = ConfigRulesStep.searchEncounterIDSAR("EncounterID");
				break;		
		}
		common.enterVisitNumber(id);
		common.clickOnSearchButton();
	}
	
	public void clicks_on_search() {	
		common.clickUpperHeaderLink("Search");
	}
